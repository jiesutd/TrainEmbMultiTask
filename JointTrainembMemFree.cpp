...
void Labeler::train(const string& atrainFile, const string& adevFile, const string& atestFile,
  ...
  int bInstanceDump = 10*atrainInsts.size();
  int bInstancePosition = m_pipe.readInstancesIndex(btrainFile, btrainInsts,0, bInstanceDump);
  if (bdevFile != "")
  ...
  int cInstanceDump = 1*atrainInsts.size();
  int cInstancePosition = m_pipe.readInstancesIndex(ctrainFile, ctrainInsts,0, cInstanceDump);
  if (cdevFile != "")
 ...
 m_pipe.readInstances(etrainFile, etrainInsts, m_options.maxInstance);
  ...
  for (int iter = 0; iter < m_options.maxIter; ++iter) {
    ...
    // reload instance and init example ,index for new iteration
    if (iter > 0) {
      cout << "Reload btrainfile/ctrainfile at iter:"<< iter <<endl;
      bInstancePosition = m_pipe.readInstancesIndex(btrainFile, btrainInsts,bInstancePosition, bInstanceDump);
      btrainExamples.clear();
      initialExamples(btrainInsts, btrainExamples, m_B_labelAlphabet);
      binputSize = btrainExamples.size();
      if (binputSize == 0) {
        // if run into instance end, then read from begining
        bInstancePosition = m_pipe.readInstancesIndex(btrainFile, btrainInsts,0, bInstanceDump);
        btrainExamples.clear();
        initialExamples(btrainInsts, btrainExamples, m_B_labelAlphabet);
        binputSize = btrainExamples.size();
      }
      ...
      BdivideA = ceil((binputSize+0.0)/ainputSize);
      //reload c
      cInstancePosition = m_pipe.readInstancesIndex(ctrainFile, ctrainInsts,cInstancePosition, cInstanceDump);
      ctrainExamples.clear();
      initialExamples(ctrainInsts, ctrainExamples, m_C_labelAlphabet);
      cinputSize = ctrainExamples.size();
      if (cinputSize == 0) {
        // if run into instance end, then read from begining
        cInstancePosition = m_pipe.readInstancesIndex(ctrainFile, ctrainInsts,0, cInstanceDump);
        ctrainExamples.clear();
        initialExamples(ctrainInsts, ctrainExamples, m_C_labelAlphabet);
        cinputSize = ctrainExamples.size();
      }
      ...
      CdivideA = ceil((cinputSize+0.0)/ainputSize);
      cout << "Btrain/Ctrain dump examples num:"<< binputSize <<"/"<< cinputSize<< endl;
    } 
    // finish reload b train instances/examples
