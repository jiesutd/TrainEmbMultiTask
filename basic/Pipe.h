...
public:
  ...
  int readInstancesIndex(const string& m_strInFile, vector<Instance>& vecInstances, int startInstance = 0, int maxInstance = -1) {
    cout << "ReadInstancesIndex:: start:"<< startInstance << ", size:"<< maxInstance << ", file:"<< m_strInFile<< endl;
    vecInstances.clear();
    initInputFile(m_strInFile.c_str());
    ...
    while (pInstance) {
      ...
      if (pInstance->size() < 512) {
        InstancePosition ++;
        if (InstancePosition > startInstance) {
          Instance trainInstance;
          trainInstance.copyValuesFrom(*pInstance);
          vecInstances.push_back(trainInstance);
          numInstance++;
